"""Build the P3.6o-7 dual-weak pair bundle on 0|1|2|3|4@gnb_2."""
...
DST = ROOT / "p3_6o7_dual_weak_pair_bundle"
TARGET_GNB = "gnb_2"
...
def main() -> None:
    ...
    for rel_dir, filename in [("bundle", "rb_rates.csv"), ("radio", "radio_rbs.csv")]:
        ...
        for row in rows:
            if row.get("scenario_id") not in target_scenario_ids:
                continue
            if row["ue_id"] not in {"3", "4"}:
                continue
            rate = float(row["rate_kbps"])
            row["rate_kbps"] = f"{max(1.0, rate * _rate_factor(row['ue_id'], rate)):.6f}"
            modified += 1
    ...
    for row in user_rows:
        ...
        if row["ue_id"] == "3":
            adjusted = _clip_cqi(current - 0.4)
            ...
            row["cqi_t_minus_3"] = f"{_clip_cqi(adjusted + 0.1):.2f}"
            row["cqi_t_minus_2"] = f"{_clip_cqi(adjusted - 0.4):.2f}"
            row["cqi_t_minus_1"] = f"{_clip_cqi(adjusted - 1.1):.2f}"
            pq_modified += 1
            history_modified += 1
            cqi_modified += 1
        elif row["ue_id"] == "4":
            adjusted = _clip_cqi(current - 0.9)
            row["previous_quality"] = "0"
            row["cqi_now_raw"] = f"{adjusted:.2f}"
            row["cqi_now"] = str(int(round(adjusted)))
    ...
    metadata["postprocess_variant"] = {
        "name": "p3_6o7_dual_weak_pair_bundle",
        "base_bundle": "p3_6i2_coupled_bundle",
        "target_family": f"{TARGET_UE_IDS}@{TARGET_GNB}",
        "window_s": [WINDOW_START_S, WINDOW_END_S],
        "intent": (
            "turn ue3 and ue4 into a true dual-weak pair while preserving the "
            "same positive-gain family, testing whether teacher finally selects "
            "{ue3, ue4} instead of ue3-only or arbitrary tie splits"
        ),
        "previous_quality_override": {"3": 0, "4": 0, "0": 2, "1": 2, "2": 2},
        "weak_pair_ue_ids": ["3", "4"],
    }
    metadata_path.write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")

    print("P3.6o-7 dual-weak pair bundle:")
