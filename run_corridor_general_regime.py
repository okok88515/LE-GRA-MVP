"""Mine and evaluate a more generic corridor regime where CQI is useful but incomplete."""
...
def is_informative_enough(row: dict, args) -> bool:
    return (
        row["teacher_groups"] >= 2
        and row["cqi_span"] >= args.min_cqi_span
        ...
        and row["max_same_cqi_cost_std"] >= args.min_same_cqi_cost_std
    )
...
def acceptance_score(row: dict) -> float:
    return float(
        ...
        + 0.9 * max(row["resource_gain_vs_cqi"], 0.0)
        + 0.8 * max(row["multifeature_gain_vs_cqi"], 0.0)
        + 0.45 * row["teacher_gain_vs_no_grouping"]
        + 0.35 * row["max_same_cqi_cost_std"]
        + 0.02 * row["cqi_span"]
    )
...
def mine_scenarios(
    *,
    ...
    while attempts < max_attempts:
        ...
        row["seed"] = seed
        row["accepted"] = int(is_informative_enough(row, args))
        row["acceptance_score"] = acceptance_score(row)
        audit_rows.append(row)
        if row["accepted"]:
            candidates.append((row["acceptance_score"], scenario, row))
            progress(
                f"[{split_name}] corridor candidate {len(candidates)} after {attempts} tries: "
                f"teacher-cqi gap={row['teacher_gain_vs_cqi']:.4f}, "
                f"resource-cqi gap={row['resource_gain_vs_cqi']:.4f}, "
                f"cqi_span={row['cqi_span']}, same-cqi-cost-std={row['max_same_cqi_cost_std']:.4f}"
            )
            if len(candidates) >= target_count * args.target_buffer_multiplier:
                break
    if len(candidates) < target_count:
        raise RuntimeError(
            f"Only mined {len(candidates)} / {target_count} {split_name} corridor scenarios "
    ...
    progress(
        f"[{split_name}] selected top {len(kept)} / {len(candidates)} corridor candidates "
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("corridor_general_regime"))
    parser.add_argument("--train-scenarios", type=int, default=64)
    ...
    parser.add_argument("--rb-budget-ratio", type=float, default=0.42)
    parser.add_argument("--min-teacher-gain", type=float, default=0.010)
    parser.add_argument("--min-cqi-gap", type=float, default=0.006)
    parser.add_argument("--min-same-cqi-cost-std", type=float, default=0.004)
    parser.add_argument("--min-cqi-span", type=int, default=3)
    parser.add_argument("--max-cqi-span", type=int, default=6)
    ...
    progress("Mining generic corridor train/test scenarios")
    ...
    progress("Training LE-GRA on corridor-general scenarios")
    model = train_model(
        train,
        test,
        feature_mode=args.feature_mode,
        ...
        progress_label="corridor_general",
    )

    progress("Evaluating methods on corridor-general test set")
