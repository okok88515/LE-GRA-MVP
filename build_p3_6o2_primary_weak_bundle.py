"""Build the P3.6o-2 primary-weak plus light-decoy bundle on 2|3|4|5@gnb_2."""
...
DST = ROOT / "p3_6o2_primary_weak_bundle"
TARGET_GNB = "gnb_2"
TARGET_UE_IDS = "2|3|4|5"
WINDOW_START_S = 24.0
WINDOW_END_S = 24.9
...
def _ue3_factor(rate_kbps: float) -> float:
    if rate_kbps >= 1128.0:
        return 0.97
    if rate_kbps >= 984.0:
        return 0.95
...
def main() -> None:
    ...
    for rel_dir, filename in [("bundle", "rb_rates.csv"), ("radio", "radio_rbs.csv")]:
        ...
        for row in rows:
            ...
            if row["ue_id"] == "4":
                ...
                row["rate_kbps"] = f"{max(1.0, rate * _ue3_factor(rate)):.6f}"
                modified += 1
    ...
    for row in user_rows:
        ...
        if row["ue_id"] == "4":
            ...
            row["cqi_t_minus_1"] = f"{_clip_cqi(current - 0.7):.2f}"
            pq_modified += 1
            history_modified += 1
        elif row["ue_id"] in {"2", "5"}:
    ...
    metadata["postprocess_variant"] = {
        "name": "p3_6o2_primary_weak_bundle",
        "base_bundle": "p3_6i2_coupled_bundle",
        "target_family": f"{TARGET_UE_IDS}@{TARGET_GNB}",
        "window_s": [WINDOW_START_S, WINDOW_END_S],
        "intent": (
            "make ue4 the clearly split-worthy primary weak user while turning "
            "ue3 into a lighter temporal decoy inside the new 2|3|4|5 family"
        ),
        "rate_transforms": {
            "ue4": {">=1128_kbps": 0.76, ">=984_kbps": 0.72, "else": 0.84},
            "ue3": {">=1128_kbps": 0.97, ">=984_kbps": 0.95, "else": 0.98},
        },
        "previous_quality_override": {"4": 0, "3": 1, "2": 2, "5": 2},
        "history_patterns": {
            "ue4": "recent_decline_primary_weak",
            "ue3": "late_drop_light_decoy",
        },
    }
    metadata_path.write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")

    print("P3.6o-2 primary-weak bundle:")
