"""Build the P3.6l-4 primary-weak plus light-decoy bundle."""
...
DST = ROOT / "p3_6l4_primary_weak_bundle"
TARGET_GNB = "gnb_2"
...
def _ue2_factor(rate_kbps: float) -> float:
    # Light decoy only: barely perturb cost geometry.
    if rate_kbps >= 1128.0:
        return 0.985
    if rate_kbps >= 984.0:
        return 0.975
...
def main() -> None:
    ...
    metadata["postprocess_variant"] = {
        "name": "p3_6l4_primary_weak_bundle",
        "base_bundle": "p3_6i2_coupled_bundle",
        "target_family": f"{TARGET_UE_IDS}@{TARGET_GNB}",
        "window_s": [WINDOW_START_S, WINDOW_END_S],
        "intent": (
            "keep ue 4 as the clearly split-worthy primary weak user while "
            "turning ue 2 into a lighter temporal decoy that should confuse "
            "static clustering more than teacher split economics"
        ),
        "rate_transforms": {
            "ue4": {">=1128_kbps": 0.82, ">=984_kbps": 0.78, "else": 0.88},
            "ue2": {">=1128_kbps": 0.985, ">=984_kbps": 0.975, "else": 0.99},
        },
        "previous_quality_override": {"4": 0, "2": 2, "1": 2, "5": 2},
        "history_patterns": {
            "ue4": "recent_decline_primary_weak",
            "ue2": "late_drop_light_decoy",
        },
    }
    metadata_path.write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")

    print("P3.6l-4 primary-weak bundle:")
