"""Build a dual-candidate targeted bundle on the 1|2|4|5 @ gnb_2 family."""
...
DST = ROOT / "p3_6l3_dual_candidate_bundle"
TARGET_GNB = "gnb_2"
...
def main() -> None:
    ...
    for row in user_rows:
        ...
        if row["ue_id"] == "4":
            ...
            row["cqi_t_minus_1"] = f"{_clip_cqi(current + 0.4):.2f}"
            pq_modified += 1
            history_modified += 1
    ...
    metadata["postprocess_variant"] = {
        "name": "p3_6l3_dual_candidate_bundle",
        "base_bundle": "p3_6i2_coupled_bundle",
        "target_family": f"{TARGET_UE_IDS}@{TARGET_GNB}",
        "window_s": [WINDOW_START_S, WINDOW_END_S],
        "intent": (
            "create a broader dual-candidate regime where ue 4 and ue 2 both "
            "look weak, but ue 4 is the primary isolation target while ue 2 "
            "acts as a competing decoy candidate"
        ),
        "rate_transforms": {
            "ue4": {">=1128_kbps": 0.84, ">=984_kbps": 0.80, "else": 0.90},
            "ue2": {">=1128_kbps": 0.94, ">=984_kbps": 0.90, "else": 0.96},
        },
        "previous_quality_override": {"4": 0, "2": 1, "1": 2, "5": 2},
        "history_patterns": {
            "ue4": "recent_decline_primary_weak",
            "ue2": "mild_recovery_decoy_weak",
        },
    }
    metadata_path.write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")

    print("P3.6l-3 dual-candidate bundle:")
