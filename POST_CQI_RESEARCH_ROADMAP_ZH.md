...
## 方向四（規模驗證，非資訊缺口）：真實資料的使用者規模 — 2026-08-27 Phase 1(SUMO路網/路線)完成，詳見 REAL_SIMU5G_SCALE_PILOT.md

Phase 1結果：原本24台車的路網其實是`netgenerate`演算法產生的(檔頭有留原始
指令)，不是手畫的，用同一工具重新產生風險比預期低很多。選擇「維持同一
3×3路網拓樸、只拉長路口間距(200m→400m)」而非「加更多路口」，因為後者
會讓路線多穿過原本沒有的紅綠燈路口，改變設計本質。放大到800×800公尺、
100台車後，headless SUMO驗證(不用跑昂貴的OMNeT++)抓到一個真實bug：車輛
從靜止插入模擬，100台車在目前的depart間隔下會有近30%卡住插不進去——用
SUMO標準解法`departSpeed="max"`修好，100台全部成功插入。gNB位置、發射
功率目前只是按比例沿用，**還沒經過Phase 2的CQI histogram校準**，這是
下一步。
