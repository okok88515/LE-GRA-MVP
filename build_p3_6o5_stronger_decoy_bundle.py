"""Build the P3.6o-5 stronger-decoy bundle on 0|1|2|3|4@gnb_2."""
...
DST = ROOT / "p3_6o5_stronger_decoy_bundle"
TARGET_GNB = "gnb_2"
...
def main() -> None:
    ...
    for row in user_rows:
        ...
        if row["ue_id"] == "3":
            ...
            row["cqi_t_minus_4"] = f"{_clip_cqi(current + 1.1):.2f}"
            row["cqi_t_minus_3"] = f"{_clip_cqi(current + 1.0):.2f}"
            row["cqi_t_minus_2"] = f"{_clip_cqi(current + 0.6):.2f}"
    ...
    metadata["postprocess_variant"] = {
        "name": "p3_6o5_stronger_decoy_bundle",
        "base_bundle": "p3_6i2_coupled_bundle",
        "target_family": f"{TARGET_UE_IDS}@{TARGET_GNB}",
        "window_s": [WINDOW_START_S, WINDOW_END_S],
        "intent": (
            "strengthen ue4 from light decoy to stronger boundary decoy while "
            "keeping ue3 as the true weak user, testing whether split structure "
            "changes before the original positive-gain basin collapses"
        ),
        "rate_transforms": {
            "ue4": {">=984_kbps": 0.94, ">=808_kbps": 0.92, "else": 0.96},
        },
        "previous_quality_override": {"3": 1, "4": 0, "0": 2, "1": 2, "2": 2},
        "history_patterns": {
            "ue3": "primary_weak_recent_decline",
            "ue4": "stronger_boundary_decoy",
        },
    }
    metadata_path.write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")

    print("P3.6o-5 stronger-decoy bundle:")
