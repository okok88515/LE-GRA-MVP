"""Build the P3.6o-8 localized gain-recovery bundle around o6."""
...
SRC = ROOT / "p3_6o6_pair_candidate_bundle"
DST = ROOT / "p3_6o8_gain_recovery_bundle"
TARGET_GNB = "gnb_2"
...
def main() -> None:
    ...
    for rel_dir, filename in [("bundle", "rb_rates.csv"), ("radio", "radio_rbs.csv")]:
        ...
        for row in rows:
            ...
            if row["ue_id"] == "0":
                row["rate_kbps"] = f"{max(1.0, rate * 1.06):.6f}"
                modified += 1
            elif row["ue_id"] == "4":
                row["rate_kbps"] = f"{max(1.0, rate * 0.94):.6f}"
                modified += 1
    ...
    for row in user_rows:
        ...
        if row["ue_id"] == "0":
            adjusted = _clip_cqi(current + 0.7)
            ...
            row["cqi_t_minus_1"] = f"{_clip_cqi(adjusted - 1.5):.2f}"
            pq_modified += 1
            history_modified += 1
            cqi_modified += 1
        elif row["ue_id"] == "3":
    ...
    metadata["postprocess_variant"] = {
        "name": "p3_6o8_gain_recovery_bundle",
        "base_bundle": SRC.name,
        "target_family": f"{TARGET_UE_IDS}@{TARGET_GNB}",
        "window_s": [WINDOW_START_S, WINDOW_END_S],
        "intent": (
            "preserve o6's structure-shift pressure, but pull ue0 back toward "
            "the strong cluster and deepen ue4 slightly so the tie split may "
            "turn into a positive-gain weak-pair regime"
        ),
    }
    metadata_path.write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")

    print("P3.6o-8 gain-recovery bundle:")
