"""Build P3.6n-7 by extending the late weak-pair regime with partial masking."""
...
DST = ROOT / "p3_6n7_extended_pair_bundle"
...
def main() -> None:
    ...
    for rel_dir, filename in [("bundle", "rb_rates.csv"), ("radio", "radio_rbs.csv")]:
        ...
        for row in rows:
            ...
            if ue_id == "4":
                factor = 0.68 if rate >= 500.0 else 0.74
                row["rate_kbps"] = f"{max(1.0, rate * factor):.6f}"
            elif ue_id == "5":
                factor = 0.82 if rate >= 360.0 else 0.88
                row["rate_kbps"] = f"{max(1.0, rate * factor):.6f}"
            elif ue_id in {"3", "6"}:
    ...
    for row in user_rows:
        ...
        if ue_id == "4":
            adjusted = max(8.9, min(9.7, current - 0.2))
            row["previous_quality"] = "2"
            row["cqi_now_raw"] = f"{adjusted:.2f}"
            row["cqi_now"] = str(int(round(adjusted)))
            row["cqi_t_minus_4"] = f"{_clip_cqi(adjusted - 1.5):.2f}"
            row["cqi_t_minus_3"] = f"{_clip_cqi(adjusted - 1.1):.2f}"
            row["cqi_t_minus_2"] = f"{_clip_cqi(adjusted - 0.8):.2f}"
            row["cqi_t_minus_1"] = f"{_clip_cqi(adjusted - 0.4):.2f}"
            touched += 1
        elif ue_id == "5":
            adjusted = max(9.0, min(9.8, current + 0.1))
            row["previous_quality"] = "2"
            row["cqi_now_raw"] = f"{adjusted:.2f}"
            row["cqi_now"] = str(int(round(adjusted)))
            row["cqi_t_minus_4"] = f"{_clip_cqi(adjusted - 1.2):.2f}"
            row["cqi_t_minus_3"] = f"{_clip_cqi(adjusted - 0.9):.2f}"
            row["cqi_t_minus_2"] = f"{_clip_cqi(adjusted - 0.6):.2f}"
            row["cqi_t_minus_1"] = f"{_clip_cqi(adjusted - 0.3):.2f}"
            touched += 1
        elif ue_id in {"3", "6"}:
            adjusted = max(12.2, current - 0.5)
    ...
    metadata["postprocess_variant"] = {
        "name": "p3_6n7_extended_pair_bundle",
        "base_bundle": SRC.name,
        "target_family": f"{TARGET_UE_IDS}@{TARGET_GNB}",
        "late_start_s": LATE_START_S,
        "late_end_s": LATE_END_S,
        "intent": (
            "extend the late {ue4, ue5} weak-pair window with stronger pressure "
            "but only partial masking, so teacher-positive pair structure stays "
            "alive longer without collapsing back to the fully obvious n5 pattern"
        ),
    }
    metadata_path.write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")

    print("P3.6n-7 extended-pair bundle:")
