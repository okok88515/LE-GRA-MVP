"""Build the P3.6o-6 pair-candidate bundle on 0|1|2|3|4@gnb_2."""
...
DST = ROOT / "p3_6o6_pair_candidate_bundle"
TARGET_GNB = "gnb_2"
...
def main() -> None:
    ...
    metadata["postprocess_variant"] = {
        "name": "p3_6o6_pair_candidate_bundle",
        "base_bundle": "p3_6i2_coupled_bundle",
        "target_family": f"{TARGET_UE_IDS}@{TARGET_GNB}",
        "window_s": [WINDOW_START_S, WINDOW_END_S],
        "intent": (
            "push ue4 from boundary decoy into a true pair-candidate while "
            "keeping ue3 as the primary weak user, testing whether teacher "
            "upgrades from ue3-only to {ue3, ue4} weak-group structure"
        ),
        "rate_transforms": {
            "ue4": {">=984_kbps": 0.84, ">=808_kbps": 0.80, "else": 0.88},
        },
        "previous_quality_override": {"3": 1, "4": 0, "0": 2, "1": 2, "2": 2},
        "history_patterns": {
            "ue3": "primary_weak_recent_decline",
            "ue4": "true_pair_candidate_decline",
        },
        "ue4_cqi_now_shift": -1.2,
    }
    metadata_path.write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")

    print("P3.6o-6 pair-candidate bundle:")
